...
* Thanks to Arun Gupta (@arungupta) for the slides
====

[NOTE ]
[role="speaker"]
====
* My name is James Perkins, and I work on WildFly and JBoss EAP for JBoss at Red Hat
====
...
[role="speaker"]
====
* (JRebel) What we’re looking at in this part of the report are the real developer concerns and metrics including:
...
[role="speaker"]
====
* WildFly is the name of the community offering, while JBoss Enterprise Application Platform is the name of the commercial offering. After JBoss AS 7.1.1 the community offering was renamed to help differentiate the two projects.
** WildFly will meet the community demands with a more rapid release cycle and the latest in innovation.
** EAP will remain more conservative to meet the demands of mission critical deployments.
* WildFly provides a lightweight managed application runtime. It has a unified management layer that is fully integrated with all aspects of server and is exposed using a variety of APIs and protocols. These could be used to automate operational needs of any environment – small JVM on a tiny server or thousands of JVM spread across the world.
* For JBoss, developer is the key. From the very beginning WildFly is designed to be easy and quick to get going. It offers rapid boot time, excellent developer tooling through not only JBoss Tools, but also NetBeans and IntelliJ. There are also a wide variety of quick start examples.
* WildFly implements the latest Java EE standards as well as additional open source innovative APIs. While Java EE is the most common usage of WildFly, it is not tied to any particular language or SDK. For example, TorqueBox provides full Ruby-on-Rails experience and is based on WildFly architecture and uses many of its core features.
* WildFly is completely open source and available to you for free. If you have an issue with your application, you can attach a debugger and find out what might be causing the issue. You can file a bug, and then send a pull request for the patch to be applied.
====
...
[role="speaker"]
====
* Java EE 7 is the latest release of the platform and focuses around increasing productivity and embracing HTML5. WildFly 8 is Java EE 7 certified and so you get all the technologies from the platform here.
* WebSocket (JSR 356), Batch (JSR 352), JSON (JSR 353), Concurrency Utilities for Java EE (JSR 236) are four new technologies in the platform. JAX-RS 2 (JSR 339) and JMS 2 (JSR 343) went through significant updates. CDI 1.1 (JSR 346), Bean Validation 1.1 (JSR 349), JPA 2.1 (JSR 338), JTA 1.2 (JSR 907) also made updates.
...
[role="speaker"]
====
* The Java API for WebSocket, WebSocket 1.0, is a key technology for HTML5 support. WebSocket provides a bi-directional and full-duplex communication channel over a single TCP connection.
* Adding `@ServerEndpoint` annotation on a POJO converts it into a WebSocket server endpoint. No additional deployment descriptors are required. The URI at which the endpoint is published is included in the annotation.
...
* First parameter receives the payload, second parameter is the session used to communicate with the client.
...
[role="speaker"]
====
* Batch Processing provides item-oriented (aka chunk) and task-oriented (aka batchlet) processing. This is a job description of chunk processing, aka Job XML.
* Job XML typically consists of many steps, this canonical sample contains one chunk step.
* This chunk has a reader, processor (optional), and writer. The ref attribute refers to CDI resolvable bean name bundled with the archive.
...
* API provides 4 types of managed objects.
...
* `JMSContext` combines in a single object the functionality of both the `Connection` and the `Session` in the earlier JMS APIs. A container-managed `JMSContext` instance can be obtained by simply injecting it with the `@Inject` annotation. Notice, no `ConnectionFactory` is specified here. In this case, a default JMS ConnectionFactory is used and this is defined by the platform.
...
* Finally, the message is sent using method chaining. For example, create a producer using `createProducer()` and then calling the `send()` method to send a message to a destination.
* Really simple and clean. This also improves readability of your code.
====
...
[role="speaker"]
====
* Undertow is a flexible performant web server written in Java, providing both true blocking and non-blocking API’s based on NIO. It has the ability to scale to millions connections and has impressive numbers on throughput. It also allows us to add support for modern security standards. And since web servers are one of the most critical pieces of an application server, writing it a fresh allows us to provide the best possible performance and memory efficiency.
...
** _8080_ for applications with JNDI and EJB multiplexed
...
[role="speaker"]
====
* By utilizing HTTP upgrade, WildFly 8 has moved nearly all of its protocols to be multiplexed over two HTTP ports: a management port, and an application port. This is a big benefit to cloud providers (such as OpenShift) who run hundreds to thousands of instances on a single server. This is another example of why WildFly is designed for the cloud / PaaS. The better examples are small memory footprint and fast startup (delivered in AS 7).
* Note that since dropped the native management port 9999, which instead uses the HTTP management port (9990). Our full profile configuration also dropped the native JMS ports 5445 and 5455, which instead are multiplexed over port 8080. However, all of these original ports can still be enabled if desired.
...
* This allows to add new subsystems, add new deployment types, your application needs to take advantage of the powerful CLI for management aspects, etc. This is nothing to do with an existing standard like Java EE or anything else, its purely related to how WildFly internals work.
* Management, classloading (JBoss Modules), logging etc is bare bones, everything else like EJB, JPA, Clustering are extensions to that. Core has a deployment manager but it does not know what to do with the uploaded archive, such as WAR, unless an extension is used.
...
* CMP - JPA offers much more performant and flexible API. JAX-RPC - JAX-WS offers a much more accurate and complete solution. JSR-88 - Very little adoption. Most preferred the more complete deployment APIs provided by vendors. We decided to completely remove support for these technologies due to the high maintenance cost, low community interest, and much better alternative solutions. If you are not able to port at this time, we recommend looking at JBoss EAP6, which provides long term maintenance and support of these technologies.
====
...
[role="speaker"]
====
* Standalone instance is a single independent instance of server, just like AS 4, 5, and 6. It can be started using `bin/standalone.sh|bat` scripts.
...
[role="speaker"]
====
* Host controller (HC) is started with domain.sh|bat script and is only concerned with server management. Only starts/stops individual server processes that run on its host, interacts with DC to help manage them.
* Domain Controller (DC) is a special HC that is configured to be the central management control point for the collection of servers in the ‘domain’.
...
* All configuration data is persisted in the backing XML configuration files.
====
